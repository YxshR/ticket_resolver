'use client'

export default function ErrorDisplay({ 
  error, 
  onRetry = null, 
  title = 'Something went wrong',
  type = 'error',
  className = '',
  showDetails = false 
}) {
  const typeStyles = {
    error: {
      container: 'bg-red-50 border-red-200',
      icon: 'text-red-400',
      title: 'text-red-800',
      message: 'text-red-700',
      button: 'bg-red-100 hover:bg-red-200 text-red-800 focus:ring-red-500'
    },
    warning: {
      container: 'bg-yellow-50 border-yellow-200',
      icon: 'text-yellow-400',
      title: 'text-yellow-800',
      message: 'text-yellow-700',
      button: 'bg-yellow-100 hover:bg-yellow-200 text-yellow-800 focus:ring-yellow-500'
    },
    info: {
      container: 'bg-blue-50 border-blue-200',
      icon: 'text-blue-400',
      title: 'text-blue-800',
      message: 'text-blue-700',
      button: 'bg-blue-100 hover:bg-blue-200 text-blue-800 focus:ring-blue-500'
    }
  }

  const styles = typeStyles[type] || typeStyles.error

  const getIcon = () => {
    switch (type) {
      case 'warning':
        return (
          <svg className={`h-5 w-5 ${styles.icon}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        )
      case 'info':
        return (
          <svg className={`h-5 w-5 ${styles.icon}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
        )
      default:
        return (
          <svg className={`h-5 w-5 ${styles.icon}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
          </svg>
        )
    }
  }

  const errorMessage = typeof error === 'string' ? error : error?.message || 'An unexpected error occurred'

  return (
    <div className={`rounded-md border p-4 ${styles.container} ${className}`}>
      <div className="flex">
        <div className="flex-shrink-0">
          {getIcon()}
        </div>
        <div className="ml-3 flex-1">
          <h3 className={`text-sm font-medium ${styles.title}`}>
            {title}
          </h3>
          <div className={`mt-2 text-sm ${styles.message}`}>
            <p>{errorMessage}</p>
          </div>
          
          {showDetails && error?.stack && process.env.NODE_ENV === 'development' && (
            <details className="mt-3">
              <summary className={`cursor-pointer text-xs ${styles.message} hover:underline`}>
                Show error details
              </summary>
              <pre className={`mt-2 text-xs ${styles.message} bg-white bg-opacity-50 p-2 rounded overflow-auto max-h-32`}>
                {error.stack}
              </pre>
            </details>
          )}
          
          {onRetry && (
            <div className="mt-4">
              <button
                type="button"
                onClick={onRetry}
                className={`text-sm px-3 py-2 rounded-md font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${styles.button}`}
              >
                Try Again
              </button>
            </div>
          )}
        </div>
      </div>
    </div>
  )
}

// Preset error components for common use cases
export function NetworkErrorDisplay({ onRetry }) {
  return (
    <ErrorDisplay
      error="Unable to connect to the server. Please check your internet connection and try again."
      title="Network Error"
      onRetry={onRetry}
      type="error"
    />
  )
}

export function NotFoundErrorDisplay({ message = "The requested resource could not be found." }) {
  return (
    <ErrorDisplay
      error={message}
      title="Not Found"
      type="warning"
    />
  )
}

export function ValidationErrorDisplay({ errors = [] }) {
  const errorMessage = Array.isArray(errors) && errors.length > 0
    ? errors.join(', ')
    : typeof errors === 'string' && errors.trim()
    ? errors 
    : 'Please check your input and try again.'

  return (
    <ErrorDisplay
      error={errorMessage}
      title="Validation Error"
      type="warning"
    />
  )
}